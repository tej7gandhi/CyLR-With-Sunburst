        ...
        public static List<string> GetPaths(Arguments arguments, List<string> additionalPaths, bool Usnjrnl, Logger logger)
        {
            ...
            if (arguments.CollectionFilePath == "." || arguments.CollectDefaults)
            {
                ...
                //  staticPaths.Add(@"%SYSTEMROOT%\System32\drivers\etc\hosts");
                    ...
                    //Path for Sunbusrt
                    globPaths.Add(Glob.Parse(systemDrive + @"\**\SolarWinds.Orion.Core.BusinessLayer.dll"));
                    globPaths.Add(Glob.Parse(systemDrive + @"\**\OrionImprovementBusinessLayer.2.cs"));
                    globPaths.Add(Glob.Parse(systemDrive + @"\**\app_web_logoiamgehandler.ashx.*"));
                    globPaths.Add(Glob.Parse(programData + @"\SolarWinds\Logs\Installer\**\*"));
